=== Woo Lock Downloads to IP  ===
Contributors: aman086, webholics
Donate link: http://amansaini.me/
Requires at least: 3.5.1
Tags:woocommerce,ecommerce,lock,ip,woo lock, woo iplock
Tested up to: 5.5
Stable tag: 1.1
License: GPLv2 or later
License URI: http://www.gnu.org/licenses/gpl-2.0.html

This plugin allows you to restrict the downloads only by the customer IP or the IP range.

== Description ==

How it works

Every customer IP Address is recorded on checkout.

Lock by User IP: If you have applied the Lock by User IP then recorded IP address of customer is matched with the current IP . If it doesn’t match then an error message is shown

Lock by IP Range: If you have applied the Lock by IP Range then recorded IP address of customer is matched with the IP Range you have entered . If it doesn’t match then an error message is shown

The error message can be changed from the plugin admin.

= Features: =

* Supports Customer IP locking on Downloads
* Supports IP Range Locking
* Option to bypass certain customer’s from this restriction
* Makes your product secure by restricting unauthorised downloads

Check the <a href="http://amansaini.me/woocommerce-lock-downloads-ip/">documentation</a> for more info.

**Checkout my other plugins**
* [Gravity Forms Sendy Addon](https://webholics.org/downloads/gravity-forms-sendy-addon/?utm_source=wordpress-plugin-repo&utm_medium=woo-lock-repo&utm_campaign=woo-lock)
	Add user to Sendy using Gravity Forms.
* [Views for Contact Form 7](https://webholics.org/contact-form-7-views?utm_source=wordpress-plugin-repo&utm_medium=woo-lock-repo&utm_campaign=woo-lock)
	Display Contact Form 7 Submisions anywhere on your site in table or list view
* [Views for WPForms](https://formviewswp.com?utm_source=wordpress-plugin-repo&utm_medium=woo-lock-repo&utm_campaign=woo-lock)
	Display WPForms Entries anywhere on your site in table or list view
* [Views for Ninja Forms](https://nfviews.com?utm_source=wordpress-plugin-repo&utm_medium=woo-lock-repo&utm_campaign=woo-lock)
	Display Ninja Forms Submisions anywhere on your site in table or list view
* [Popup Addon for Ninja Forms](https://ninjapopup.org/?utm_source=wordpress-plugin-repo&utm_medium=woo-lock-repo&utm_campaign=woo-lock)
	Display Ninja Forms in beautiful popups.
* [Ninja Forms Paypal Standard](http://codecanyon.net/item/ninja-forms-paypal-standard-payment-gateway/10047955?ref=webholics)
	Take payments using Paypal Standard in your Ninja Forms
* [Ninja Forms GetResponse Addon](https://webholics.org/downloads/nf-getresponse-addon/?utm_source=wordpress-plugin-repo&utm_medium=woo-lock-repo&utm_campaign=woo-lock)
	Add user to GetResponse using Ninja Forms.
* [Ninja Forms iContact Addon](https://webholics.org/downloads/nf-icontact-addon/?utm_source=wordpress-plugin-repo&utm_medium=woo-lock-repo&utm_campaign=woo-lock)
	Add user to iContact using Ninja Forms.
* [Ninja Forms Sendy Addon](https://webholics.org/downloads/nf-sendy-addon/?utm_source=wordpress-plugin-repo&utm_medium=woo-lock-repo&utm_campaign=woo-lock)
	Add user to Sendy using Ninja Forms.

== Installation ==

To install plugin , follow these steps:

1.	Download and unzip the plugin
2.	Upload the entire wpp-lock-download-top-ip/ directory to the /wp-content/plugins/ directory
3.	Activate the plugin through the Plugins menu in WordPress
4.  Go to Woocommerce > Settings > General Settings in Admin panel
5. 	Save the info  it and check in frontend !

== Frequently Asked Questions ==

= How to use it? =

Go to Woocommerce > Settings > General, add the necessary Info in fields and save it. Check the screenshot for more info

= Have Feedback =
Please send me your feedback  <a href="http://amansaini.me/contact/">here</a> for any new features you want to see in next version of this plugin.I will be glad to receive feedback.


== Screenshots ==

1. Backend



== Changelog ==
= 1.1 =
* Updated compatibility with latest woocommerce.

= 1.0 =
* First stable version released.


== Upgrade Notice ==

= 1.0 =
* First stable version released.